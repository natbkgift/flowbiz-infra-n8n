# Project Contract (Phase 1 + Kill-Switch)

**Scope:** Integration-only bridge per `docs/BLUEPRINT.md` Phase 1, plus Phase 2 kill-switch (PR-8).
...
- POST `/v1/jobs/{job_id}/cancel` — Kill-switch endpoint; deactivates the referenced workflow in n8n and returns `status="cancelled"` when successful.
